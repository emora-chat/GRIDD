...
def update_salience_gpu(wm):
  #  import cProfile, pstats
   ...
   # profiler.enable()
    ...
    wmc = wm.concepts()
    tsal = torch.zeros(1,len(wmc),device=device)
    tedges = torch.zeros(len(wmc), len(wmc),device=device)

    for i, name in enumerate(wmc):
        names[name] = i
        try:
            tsal[0][i]=wm.features[name][SALIENCE]
        except:
            pass
    ...
    for s, t, o, i in wm.predicates():
        if i not in SAL_FREE:
            if s not in SAL_FREE:
                tedges[names[s]][names[i]] = MY_DECAY
                tedges[names[i]][names[s]] = MY_DECAY
            if t not in SAL_FREE:
                tedges[names[t]][names[i]] = MY_DECAY
                tedges[names[i]][names[t]] = MY_DECAY
            if o is not None and o not in SAL_FREE:
                tedges[names[o]][names[i]] = MY_DECAY
                tedges[names[i]][names[o]] = MY_DECAY
        ...
        if isinstance(edge[2], tuple) and AND_LINK == edge[2][0]:
            for evidence, and_node, _ in edge:
                or_links = [edge for edge in wm.metagraph.out_edges(and_node) if
                            isinstance(edge[2], tuple) and OR_LINK == edge[2][0]]
                for _, implication, _ in or_links:
                    tedges[names[evidence]][names[implication]] = MY_DECAY


    a = torch.matmul(tsal, tedges)
...
class TestSalienceUpdate(unittest.TestCase):
    ...
    def test_update_salience_gpu(self):
        wm = ConceptGraph('''
        ...
        saliencesold = update_salience_gpu(wm)
        t2 = time.time()
        profiler.disable()
        stats = pstats.Stats(profiler).sort_stats('tottime')
        stats.print_stats()
        print("\nGpu method: {}".format(t2 - t1))
        wm = ConceptGraph('''
        ...
        saliencesnew = update_salience(wm)
        t22 = time.time()
        print("CPU method: {}".format(t22 - t21))
        #  for concept, salience in saliences.items():
        #   print(concept, ':', salience)
        # for concept, salience in saliences.items():
        #    print(concept, ':', salience)
